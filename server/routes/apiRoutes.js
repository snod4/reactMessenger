...
router.post("/removeConvo", (req, res)=>{
  if(req.isAuthenticated){
    console.log("Got to removeConvo POST");
    User.findById(req.user._id, (err, user)=>{
      if(err){
        ...
        for(let i = 0; i < user.onGoingConversations.length; i++){
          console.log(`user convoId: ${user.onGoingConversations[i].recipientId}, request convoId: ${req.body.recipientId}, ${user.onGoingConversations[i].recipientId.trim() === req.body.recipientId.trim()}`)
          if(user.onGoingConversations[i].recipientId.trim() === req.body.recipientId.trim()){
            user.onGoingConversations.splice(i, 1);
            break;
          }
        }
        User.updateOne(
